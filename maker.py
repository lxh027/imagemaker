...
def find_closiest(color, list_colors):
    ...
    for cur_color in list_colors:
        n_diff = math.sqrt(
            math.pow(math.fabs(color[0] - cur_color[0]), 2) + math.pow(math.fabs(color[1] - cur_color[1]),
                                                                       2) + math.pow(math.fabs(color[2] - cur_color[2]),
                                                                                     2))
    ...
    for y1 in range(0, height, SLICE_SIZE):
        for x1 in range(0, width, SLICE_SIZE):
            try:
                ...
                r = '\r[{}{}]{}%'.format(35 * now_done, 32 * (100 - now_done), now_done)
                sys.stdout.write(r)
                sys.stdout.flush()
